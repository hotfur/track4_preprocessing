...
def main(argv, f, file, file_color, corners_shape):
    ...
    for i in range(len(temp_corners) - 1):
        gr_lines.append(
            find_line(temp_corners[i][0], -temp_corners[i][1], temp_corners[i + 1][0], - temp_corners[i + 1][1]))
        if tuple(temp_corners[i]) not in dict_line.keys():
            dict_line[tuple(temp_corners[i])] = [
                find_line(temp_corners[i][0], -temp_corners[i][1], temp_corners[i + 1][0], - temp_corners[i + 1][1])]
        else:
            dict_line[tuple(temp_corners[i])].append(
                find_line(temp_corners[i][0], -temp_corners[i][1], temp_corners[i + 1][0], - temp_corners[i + 1][1]))
        if tuple(temp_corners[i + 1]) not in dict_line.keys():
            dict_line[tuple(temp_corners[i + 1])] = [
                find_line(temp_corners[i][0], -temp_corners[i][1], temp_corners[i + 1][0], - temp_corners[i + 1][1])]
        else:
            dict_line[tuple(temp_corners[i + 1])].append(
                find_line(temp_corners[i][0], -temp_corners[i][1], temp_corners[i + 1][0], - temp_corners[i + 1][1]))
    print("dict line")
    print(dict_line)
    for corner in corners:
    ...
    if len(corners) == 4:
        ...
        if check_line_parralell(line1, line2) and check_line_parralell(line3, line4):
            ...
            for i in range(len(hull_list)):
                ...
                # print("rec")
                cv.imwrite(
                    "D:/IU_Studying/research/AIC/Auto-retail-syndata-release/rectangle/label/" + name[0] + "_mask.jpg",
                    mask)
                cv.imwrite(
                    "D:/IU_Studying/research/AIC/Auto-retail-syndata-release/rectangle/seg/" + name[0] + "_mask.jpg",
                    cropt)
        else:
            # print(line1)
            # print(line2)
        ...
        if len(dict_cnt) > 0:
            ...
            cv.circle(cdstP, (int(list(sort_orders[0])[0][0]), abs(int(list(sort_orders[0])[0][1]))), 4, (88, 100, 0),
                      -1)
            ...
            for i in range(len(box_shapes)):
                ...
                cv.imshow("mask cropt " + str(i), cropt)
                name = f.split('.')
                cv.imwrite('D:/IU_Studying/research/AIC/Auto-retail-syndata-release/box shape/label/' + name[
                    0] + "_mask_" + str(i) + ".jpg", mask)
                cv.imwrite(
                    'D:/IU_Studying/research/AIC/Auto-retail-syndata-release/box shape/seg/' + name[0] + "_seg_" + str(
                        i) + ".jpg", cropt)
    ...
    # dst = cv.cornerHarris(cdstP,2,3,0.02)
        ...
        cropt = cv.bitwise_and(src_color, src_color, mask=src)
        cv.imshow("mask cropt " + str(i), cropt)
        cv.imwrite('D:/IU_Studying/research/AIC/Auto-retail-syndata-release/box shape/failed/' + name[0] + '.jpg',
                   cropt)
    # Threshold for an optimal value, it may vary depending on the image.
